...
<header class="sticky top-0 z-50 w-full bg-white/95 backdrop-blur-md transition-all">
	<div class="mx-auto flex h-16 sm:h-18 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
		<a href="/" class="flex flex-col group py-1">
			<span class="text-xs sm:text-sm font-bold tracking-widest text-black uppercase font-sans">
				{PROFILE_DATA.name}
			</span>
			<span class="text-[9px] sm:text-[10px] text-black font-mono tracking-wider uppercase font-semibold">
				Independent Strategic AI Advisory
			</span>
		</a>

		<!-- Desktop Navigation -->
		<nav class="hidden lg:flex items-center gap-7 font-mono text-xs uppercase tracking-wider text-black font-semibold">
	...
	{#if mobileMenuOpen}
		<div class="lg:hidden bg-white px-4 pt-3 pb-6 space-y-3 shadow-xl">
			<nav class="flex flex-col space-y-1 font-mono text-xs uppercase tracking-wider text-black font-semibold">
