...
{#if terminalState.isOpen}
	<div
		...
		tabindex="-1"
		class="fixed inset-0 z-50 bg-[#000914]/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-150"
		onclick={handleBackdropClick}
		onkeydown={handleKeydown}
	>
		<div
			class="w-full max-w-3xl bg-[#000914] border border-[#DDEBFB] shadow-2xl flex flex-col rounded-none overflow-hidden"
		>
			<!-- Header / Telemetry Bar -->
			<div
				class="border-b border-[#DDEBFB] px-4 py-2.5 flex items-center justify-between font-mono text-[10px] text-[#DDEBFB] uppercase tracking-widest bg-[#000914]"
			>
				...
				<div class="flex items-center gap-3">
					...
					{:else}
						<span class="text-[#DDEBFB]">[READY]</span>
					{/if}
					<button
						type="button"
						onclick={() => terminalState.close()}
						class="text-[#DDEBFB] hover:text-[#3300FF] transition-colors cursor-pointer"
				...
				{#each terminalState.messages as msg (msg.id)}
					{#if msg.role === 'user'}
						<div class="flex flex-col items-end space-y-1">
							<div class="flex items-center gap-2 text-[10px] text-[#DDEBFB] uppercase">
								<span>{msg.timestamp}</span>
								<span>&gt; USER:</span>
							</div>
							<div class="font-mono text-xs text-[#DDEBFB] bg-[#000914] border border-[#DDEBFB] px-3 py-2 max-w-xl text-right">
								{msg.content}
							</div>
						</div>
					{:else}
						<div class="flex flex-col items-start space-y-1">
							...
							<div class="font-sans text-sm text-[#DDEBFB] leading-relaxed whitespace-pre-wrap max-w-2xl bg-[#000914] border-l-2 border-[#3300FF] pl-3 py-1">
								{@html msg.content}
			...
			<form
				...
				class="border-t border-[#DDEBFB] p-4 flex items-center gap-3 bg-[#000914]"
			>
				...
				<input
					...
					class="w-full bg-transparent text-[#DDEBFB] font-mono text-sm outline-none placeholder:text-[#DDEBFB]/40"
				...
				<button
					...
					class="font-mono text-xs uppercase px-3 py-1.5 border border-[#3300FF] text-[#DDEBFB] bg-[#3300FF] hover:bg-[#000914] hover:text-[#DDEBFB] disabled:opacity-30 disabled:pointer-events-none transition-all cursor-pointer"
				>
					[EXEC]
				</button>
			</form>
