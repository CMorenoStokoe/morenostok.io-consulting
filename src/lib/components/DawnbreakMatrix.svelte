<script module lang="ts">
	let hasPlayedOnceGlobally = false;
</script>
...
<script lang="ts">
	...
	const DURATION_MS = 3200; // 3.2s clean dawnbreak sweep cycle
	...
	function drawFrame(ctx: CanvasRenderingContext2D, p: number, elapsed: number) {
		...
		// 3. Crystallization into pure dark dots (0.75 to 1.0)
		...
		for (let i = 0; i < dots.length; i++) {
			...
			if (p < 0.2) {
				...
				color = horizonFactor > 0.4 ? '#3300FF' : '#012148';
			} else if (p < 0.75) {
				// Wave sweeping along diagonal
				const distFromWave = d.diagProj - waveFront;

				if (distFromWave < 0) {
					// Behind wave: locking into dark dots for starburst
					if (d.isInside) {
				...
				// Final frame: clean dark dots (#012148) for starburst, faint ambient background
				if (d.isInside) {
					alpha = 1.0;
					r = 2.4;
					color = '#012148';
	...
	onMount(() => {
		...
		if (hasPlayedOnceGlobally) {
			// Already completed in this session: render final static starburst immediately
			drawFrame(ctx, 1, DURATION_MS);
		...
		function startSweep() {
			if (animationStarted || isAnimationFinished) return;
			...
			function render(now: number) {
				...
				// Sweep once and lock into final static state
				if (p < 1) {
					animId = requestAnimationFrame(render);
				} else {
					animId = null;
					isAnimationFinished = true;
					hasPlayedOnceGlobally = true;
					drawFrame(ctx, 1, DURATION_MS);
				}
			}
